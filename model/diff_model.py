...
class CrossDiTblock(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm1 = nn.LayerNorm(feature_dim, elementwise_affine=False, eps=1e-5) #1e-4
        self.attn = Attention2(feature_dim, num_heads=num_heads, qkv_bias=True, **kwargs)

        self.norm2 = nn.LayerNorm(feature_dim, elementwise_affine=False, eps=1e-5)#1e-4
        self.cross_attn = CrossAttention(feature_dim, num_heads=num_heads, qkv_bias=True, **kwargs)

        self.norm3 = nn.LayerNorm(feature_dim, elementwise_affine=False, eps=1e-5)#1e-4
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, out_size):
        ...
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-5)#1e-4
        # 投影成输出的 patch 形状
...
class DiT_diff(nn.Module):
    def __init__(self,
        ...
        self.cond_layer_2 = nn.Sequential(
            nn.Linear(768, hidden_size)
        )

        self.cond_age = nn.Sequential(
            nn.Embedding(max_age+1, hidden_size) #120
        )
        ...
        # out layer
        self.out_layer = FinalLayer(self.hidden_size, self.st_input_size)
    ...
    def forward(self, x, t, y, a, ob, stage, **kwargs):
        x = x.float()
        t = self.time_emb(t)

        ob = self.cond_layer(ob)
        # y = self.cond_layer(y)
        y = self.cond_layer_2(y)
        stage = self.cond_layer_2(stage)
        a = self.cond_age(a)
        # z = self.condi_emb(z)
        # c = t
        #c = t + y + a + ob
